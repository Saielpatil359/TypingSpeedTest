        ...
        if not db.exec(select(Text)).first():
            db.add_all(
                [
                    Text(
                        duration=60,
                        content="Short 60s paragraph. Warm up and find rhythm. Sanket is student from mulund with good physique and chest.",
                        active=True,
                    ),
                    Text(
                        duration=90,
                        content="Medium 90s paragraph. Balance speed and accuracy.",
                        active=True,
                    ),
                    Text(
                        duration=120,
                        content="Long 120s paragraph. Maintain form and consistency.",
                        active=True,
                    ),
                ]
            )
...
@app.get("/api/texts")
def get_texts(duration: int, db: Session = Depends(get_db)):
    if duration not in (60, 90, 120):
        raise HTTPException(status_code=400, detail="Invalid duration. Use 60, 90, or 120.")
    pool = db.exec(
        select(Text).where(Text.duration == duration, Text.active == True)
    ).all()
    if not pool:
...
class ResultIn(BaseModel):
    user_id: Optional[str] = None  # null or anonymous
    duration: int
    wpm: int
    accuracy: int
    correct_chars: int
...
def get_all_texts(db: Session = Depends(get_db)):
    ...
    return [
        {"id": t.id, "duration": t.duration, "content": t.content, "active": t.active}
        for t in texts
    ]
